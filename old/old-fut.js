...
export default class FutPage {
  ...
  async login() {
    ...
    let pageTitleEl;
    try {
      [, pageTitleEl] = await Promise.all([this.page.waitForNavigation(), this.page.waitForSelector(this.pageTitleSelector, { timeout: 60 * 1000 })]);
    } catch (e) {
      needs2FA = true;
    }
  ...
  async listPlayerOnTransferMarket({ name, position, rating }) {
    ...
    const startPriceSelector = '//div[contains(@class, "DetailPanel")]//div[@class="panelActionRow"][.//span[text()="Start Price:"]]';
    const [startPriceEl] = await this.page.$x(startPriceSelector);

    const buyNowSelector = '//div[contains(@class, "DetailPanel")]//div[@class="panelActionRow"][.//span[text()="Buy Now Price:"]]';
    ...
    await Promise.all([
      ...
      this.page.waitForXPath('//div[contains(@class, "DetailPanel")]//p[@class="tradeStatus"][text()="This Item is currently listed for Transfer."]'),
    ]);
